...
def run_backtest_and_export():
    ...
    """
    df = market_data.get_historical_data(None, get_security_id(config.INDEX_TO_TRADE), use_mock_data=True)
    if df is not None:
...
def main():
    ...
    if config.LIVE_TRADING:
        ...
        if dhan:
            ...
            # Use a deque to store a fixed number of candles for indicator calculation
            ...
            while True:
                ...
                if latest_candle is not None:
                    historical_data.append(latest_candle.iloc[0])
