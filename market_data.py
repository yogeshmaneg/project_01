...
def get_latest_candle(dhan, security_id, exchange_segment=config.EXCHANGE_SEGMENT[config.INDEX_TO_TRADE], instrument='INDEX'):
    """
    Fetches the latest candle for a given security ID.

    WARNING: This function is highly inefficient for live trading as it
    fetches the entire day's data on every call. Using this in a live
    environment for extended periods will likely result in API rate limiting.
    A production-ready solution should use websockets.
    """
    try:
...
def get_historical_data(dhan, security_id, exchange_segment=config.EXCHANGE_SEGMENT[config.INDEX_TO_TRADE], instrument='INDEX', interval='5', use_mock_data=True):
    """
